...
import type { Feature, FeatureEffectMetadata, SourcePack, CharacterClass, CharacterRace, BackgroundInfo, Character, SpellcastingGrantMetadata, ProficiencyGrantMetadata, ChoiceGrantMetadata, NPC, Monster } from '@character-chronicle/shared/types'; // Use shared library path
import { ALL_SKILLS, SKILL_ABILITY_MAP, SPELL_SLOTS_BY_LEVEL } from '@character-chronicle/shared/types'; // Use shared library path
import { LoggingService } from '../logging/logging.service'; // Updated path
import { RulesService } from '../rules/rules.service'; // Updated path
          ...
          try {
              ...
              switch (metadata.effectType) {
                  ...
                  case 'spellcastingGrant':
                   ...
                   case 'acBonus': case 'acCalculation': case 'advantage': case 'resistance': case 'choiceGrant': case 'attackBonus': case 'damageBonus': this.logger.debug(`Informational feature processed: ${feature.name} (${metadata.effectType})`, 'FeaturesService'); break;
