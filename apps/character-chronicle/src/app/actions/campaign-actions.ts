...
export async function createCampaignAction(campaignData: Pick<Campaign, 'name' | 'description' | 'activeSourcePackIds'>, dmId: string): Promise<{ success: boolean; campaignId?: string; error?: string }> {
  try {
    logMessage('info', `[Action] Attempting to create campaign: ${campaignData.name}`, undefined, 'CampaignActions', { dmId });
    const campaignService = await getCampaignService();
    const campaignId = await campaignService.createCampaign(campaignData, dmId);
    logMessage('info', `[Action] Campaign created successfully: ${campaignId}`, undefined, 'CampaignActions', { dmId, campaignName: campaignData.name });
    return { success: true, campaignId };
  } catch (error) {
    logError(error, { message: `[Action] Error creating campaign ${campaignData.name}`, dmId }, 'CampaignActions');
...
export async function loadCampaignAction(campaignId: string): Promise<{ success: boolean; campaign?: Campaign | null; error?: string }> {
  try {
    logMessage('debug', `[Action] Attempting to load campaign: ${campaignId}`, undefined, 'CampaignActions');
    const campaignService = await getCampaignService();
    const campaign = await campaignService.loadCampaign(campaignId);
    logMessage('debug', `[Action] Campaign ${campaignId} ${campaign ? 'loaded' : 'not found'}.`, undefined, 'CampaignActions');
    return { success: true, campaign };
  } catch (error) {
    logError(error, { message: `[Action] Error loading campaign ${campaignId}` }, 'CampaignActions');
...
export async function loadAllCampaignsAction(userId?: string, userRole?: string): Promise<{ success: boolean; campaigns: Campaign[]; error?: string }> {
  try {
    logMessage('debug', `[Action] Attempting to load campaigns for user: ${userId ?? 'N/A'}`, undefined, 'CampaignActions', { userRole });
    if (!userId) {
        logMessage('warn', '[Action] loadAllCampaignsAction called without userId.', undefined, 'CampaignActions');
    ...
    logMessage('debug', `[Action] Loaded ${campaigns.length} campaigns for user: ${userId}`, undefined, 'CampaignActions');
    return { success: true, campaigns };
  } catch (error) {
    logError(error, { message: `[Action] Error loading campaigns for user ${userId ?? 'N/A'}`, userRole }, 'CampaignActions');
...
export async function updateCampaignAction(campaignId: string, campaignData: Partial<Omit<Campaign, 'id' | 'createdAt' | 'updatedAt'>>, currentUserId: string): Promise<{ success: boolean; error?: string }> {
   try {
    logMessage('info', `[Action] Attempting to update campaign: ${campaignId}`, undefined, 'CampaignActions', { currentUserId, updateKeys: Object.keys(campaignData) });
    const campaignService = await getCampaignService();
    await campaignService.updateCampaign(campaignId, campaignData, currentUserId);
    logMessage('info', `[Action] Campaign updated successfully: ${campaignId}`, undefined, 'CampaignActions', { currentUserId });
    return { success: true };
  } catch (error) {
    logError(error, { message: `[Action] Error updating campaign ${campaignId}`, currentUserId }, 'CampaignActions');
...
export async function deleteCampaignAction(campaignId: string, currentUserId: string): Promise<{ success: boolean; error?: string }> {
  try {
    logMessage('info', `[Action] Attempting to delete campaign: ${campaignId}`, undefined, 'CampaignActions', { currentUserId });
    const campaignService = await getCampaignService();
    await campaignService.deleteCampaign(campaignId, currentUserId);
    logMessage('info', `[Action] Campaign deleted successfully: ${campaignId}`, undefined, 'CampaignActions', { currentUserId });
    return { success: true };
  } catch (error) {
    logError(error, { message: `[Action] Error deleting campaign ${campaignId}`, currentUserId }, 'CampaignActions');
...
export async function addPlayerToCampaignAction(campaignId: string, playerId: string, currentUserId: string): Promise<{ success: boolean; error?: string }> {
   try {
    logMessage('info', `[Action] Attempting to add player ${playerId} to campaign: ${campaignId}`, undefined, 'CampaignActions', { currentUserId });
    const campaignService = await getCampaignService();
    await campaignService.addPlayerToCampaign(campaignId, playerId, currentUserId);
    logMessage('info', `[Action] Player ${playerId} added to campaign ${campaignId} successfully`, undefined, 'CampaignActions', { currentUserId });
    return { success: true };
  } catch (error) {
    logError(error, { message: `[Action] Error adding player ${playerId} to campaign ${campaignId}`, currentUserId }, 'CampaignActions');
...
export async function removePlayerFromCampaignAction(campaignId: string, playerId: string, currentUserId: string): Promise<{ success: boolean; error?: string }> {
    try {
        logMessage('info', `[Action] Attempting to remove player ${playerId} from campaign: ${campaignId}`, undefined, 'CampaignActions', { currentUserId });
        const campaignService = await getCampaignService();
        await campaignService.removePlayerFromCampaign(campaignId, playerId, currentUserId);
        logMessage('info', `[Action] Player ${playerId} removed from campaign ${campaignId} successfully`, undefined, 'CampaignActions', { currentUserId });
        return { success: true };
    } catch (error) {
        logError(error, { message: `[Action] Error removing player ${playerId} from campaign ${campaignId}`, currentUserId }, 'CampaignActions');
...
export async function addGameLogEntryAction(logEntryData: Omit<GameLogEntry, 'id' | 'timestamp'>): Promise<{ success: boolean; entryId?: string; error?: string }> {
  try {
    ...
    logError(error, { message: `[Action] Error adding game log entry to campaign ${logEntryData.campaignId}`, actorId: logEntryData.actorId }, 'CampaignActions');
...
export async function loadGameLogEntriesAction(campaignId: string, limitCount?: number): Promise<{ success: boolean; entries: GameLogEntry[]; error?: string }> {
  try {
    logMessage('debug', `[Action] Attempting to load game log entries for campaign: ${campaignId}`, undefined, 'CampaignActions', { limitCount });
    const campaignService = await getCampaignService();
    const entries = await campaignService.loadGameLogEntries(campaignId, limitCount);
    logMessage('debug', `[Action] Loaded ${entries.length} game log entries for campaign: ${campaignId}`, undefined, 'CampaignActions');
    return { success: true, entries };
  } catch (error) {
    logError(error, { message: `[Action] Error loading game log entries for campaign ${campaignId}`, limitCount }, 'CampaignActions');
...
export async function saveSourcePackAction(sourcePackData: Omit<SourcePack, 'createdAt' | 'updatedAt'> & { id?: string }, currentUserId: string): Promise<{ success: boolean; packId?: string; error?: string }> {
  try {
    logMessage('info', `[Action] Attempting to save source pack: ${sourcePackData.name || sourcePackData.id || 'New Pack'}`, undefined, 'CampaignActions', { currentUserId });
    const campaignService = await getCampaignService();
    const packId = await campaignService.saveSourcePack(sourcePackData, currentUserId);
    logMessage('info', `[Action] Source pack saved successfully: ${packId}`, undefined, 'CampaignActions', { currentUserId, packName: sourcePackData.name });
    return { success: true, packId };
  } catch (error) {
    logError(error, { message: `[Action] Error saving source pack ${sourcePackData.name || sourcePackData.id || 'New Pack'}`, currentUserId }, 'CampaignActions');
...
export async function loadSourcePackAction(sourcePackId: string): Promise<{ success: boolean; pack?: SourcePack | null; error?: string }> {
  try {
    logMessage('debug', `[Action] Attempting to load source pack: ${sourcePackId}`, undefined, 'CampaignActions');
    const campaignService = await getCampaignService();
    const pack = await campaignService.loadSourcePack(sourcePackId);
    logMessage('debug', `[Action] Source pack ${sourcePackId} ${pack ? 'loaded' : 'not found'}.`, undefined, 'CampaignActions');
    return { success: true, pack };
  } catch (error) {
    logError(error, { message: `[Action] Error loading source pack ${sourcePackId}` }, 'CampaignActions');
...
export async function loadSourcePacksByCreatorAction(creatorId: string): Promise<{ success: boolean; packs: SourcePack[]; error?: string }> {
  try {
    logMessage('debug', `[Action] Attempting to load source packs for creator: ${creatorId}`, undefined, 'CampaignActions');
    const campaignService = await getCampaignService();
    const packs = await campaignService.loadSourcePacksByCreator(creatorId);
    logMessage('debug', `[Action] Loaded ${packs.length} source packs for creator: ${creatorId}`, undefined, 'CampaignActions');
    return { success: true, packs };
  } catch (error) {
    logError(error, { message: `[Action] Error loading source packs for creator ${creatorId}` }, 'CampaignActions');
...
export async function deleteSourcePackAction(sourcePackId: string, currentUserId: string): Promise<{ success: boolean; error?: string }> {
  try {
    logMessage('info', `[Action] Attempting to delete source pack: ${sourcePackId}`, undefined, 'CampaignActions', { currentUserId });
    const campaignService = await getCampaignService();
    await campaignService.deleteSourcePack(sourcePackId, currentUserId);
    logMessage('info', `[Action] Source pack deleted successfully: ${sourcePackId}`, undefined, 'CampaignActions', { currentUserId });
    return { success: true };
  } catch (error) {
    logError(error, { message: `[Action] Error deleting source pack ${sourcePackId}`, currentUserId }, 'CampaignActions');
...
export async function getCombinedContentFromPacksAction(packIds: string[]): Promise<{ success: boolean; content?: SourcePack['content']; error?: string }> {
    try {
        logMessage('debug', `[Action] Attempting to get combined content from packs: ${packIds.join(', ')}`, undefined, 'CampaignActions');
        const campaignService = await getCampaignService();
        const content = await campaignService.getCombinedContentFromPacks(packIds);
        logMessage('debug', `[Action] Successfully combined content from packs: ${packIds.join(', ')}`, undefined, 'CampaignActions');
        return { success: true, content };
    } catch (error) {
        logError(error, { message: `[Action] Error combining content from packs`, packIds }, 'CampaignActions');
