...
export async function loadUserProfileAction(userId: string): Promise<{ success: boolean; profile?: UserProfile | null; error?: string }> {
  try {
    logMessage('debug', `[Action] Attempting to load user profile: ${userId}`);
    const userService = await getUserService();
    const profile = await userService.loadUserProfile(userId);
    logMessage('debug', `[Action] User profile ${profile ? 'found' : 'not found'} for ID: ${userId}`);
    return { success: true, profile };
  } catch (error) {
    logError(error, { message: `[Action] Error loading user profile ${userId}` });
...
export async function createUserProfileAction(profileData: Omit<UserProfile, 'createdAt' | 'updatedAt'>): Promise<{ success: boolean; profile?: UserProfile; error?: string }> {
  try {
    logMessage('info', `[Action] Attempting to create user profile for ID: ${profileData.id}`, { email: profileData.email });
    const userService = await getUserService();
    const newProfile = await userService.createUserProfile(profileData);
    logMessage('info', `[Action] User profile created successfully for ID: ${profileData.id}`);
    return { success: true, profile: newProfile };
  } catch (error) {
    logError(error, { message: `[Action] Error creating user profile ${profileData.id}`, email: profileData.email });
...
export async function updateUserProfileAction(userId: string, updates: Partial<Pick<UserProfile, 'displayName' | 'role'>>): Promise<{ success: boolean; error?: string }> {
  try {
    logMessage('info', `[Action] Attempting to update user profile: ${userId}`, { updates });
    const userService = await getUserService();
    await userService.updateUserProfile(userId, updates);
    logMessage('info', `[Action] User profile updated successfully: ${userId}`);
    return { success: true };
  } catch (error) {
    logError(error, { message: `[Action] Error updating user profile ${userId}`, updates });
