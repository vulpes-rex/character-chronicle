...
import { loadUserProfileAction, createUserProfileAction } from '@/app/actions/user-actions'; // Use Server Actions
import type { UserProfile } from '@/lib/types'; // Use alias
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        ...
        // Load or create user profile using Server Actions
        const loadResult = await loadUserProfileAction(firebaseUser.uid);

        if (loadResult.success && loadResult.profile) {
            setUserProfile(loadResult.profile);
        } else if (loadResult.success && !loadResult.profile) {
             // Profile doesn't exist, create it
             console.log(`Profile not found for ${firebaseUser.uid}, creating...`);
             const createResult = await createUserProfileAction({
                id: firebaseUser.uid,
  ...
  // Optional: Show loading state while auth is initializing
  // if (loading) {
